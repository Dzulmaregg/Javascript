// JavaScript Document
        ...
        if ('parentId' in Items[i]) {
         ...
         avatar.find('.comment_avatar img').each(function () {
             ...
             $(this).show().attr('src', ava.replace(/\/s[0-9]+(\-c)?\//, "/s400-c/"))
         });        
...
$('.para').find('a.allow').filter(function() {
			...
			a1 = this.href;
			a2 = a1.split('#');
			$(this).closest('.comment_inner').html('<div class="loader" style="height:100px;background-position:50% 50%;"></div>').load(a2[0].split('?')[0]+' #'+a2[1], function() {
